...
// === SEG KIP WORKBOOK MAP API ===
app.get("/api/workbook", async (req, res) => {
  try {
    const __SHEET_ID = "191RWU_J2IxqfwdwCbvopVtcb4WhRkPM1UQppVbgiLhs";

    const __auth = new google.auth.GoogleAuth({
      keyFile: "service-account.json",
      scopes: ["https://www.googleapis.com/auth/spreadsheets.readonly"],
    });

    const __client = await __auth.getClient();
    const __sheets = google.sheets({ version: "v4", auth: __client });

    const meta = await __sheets.spreadsheets.get({
      spreadsheetId: __SHEET_ID,
      includeGridData: false,
    });

    const sheetNames = meta.data.sheets.map(s => s.properties.title);

    const values = await __sheets.spreadsheets.values.batchGet({
      spreadsheetId: __SHEET_ID,
    ...
    res.json({
      ...
      data: Object.fromEntries(
        values.data.valueRanges.map((r, i) => [sheetNames[i], r.values || []])
      )
    });
